...
export function startThemes(id) {
  ...
  themesTreeMum=createTree({[mainId]: readFolderDirectory(themesPath + '/' + mainId)});
...
function createTree(themes) {
  ...
  function innerCreate(themes, parent) {
    ...
    for (let key in themes) {
      ...
      templatesBranch.props.name=config.viewsFolderName;
      myChild.addRelationship(templatesBranch);
      createBranch(themes[key], templatesBranch, config.viewsFolderName, true);
...
export function getTpFilesList(myTheme, includeSubs=false) {
  ...
  while (myTheme) {
    let themeTps=[];
    let themePaths={};
    for (const child of myTheme.getRelationship(config.viewsFolderName).children) {
