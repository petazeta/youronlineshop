import {TmptsCache} from '../../tmptscache.mjs'
...
if (config.cache=='mongodb') { 
  ...
  cache = new TmptsCache(mongodbDriver);
}
else if (config.cache=='mem'){
  cache = new TmptsCache(new Map()) // Maybe it is better to exclude here templates to be cached => cache = new TmptsCache(new Map(), "templates")
}
  ...
  cache = new TmptsCache(redisDriver);
}

export function cacheRequest(contentType, parameters) {
  if (config.cache=='none') return false;
  return cache.request(contentType, parameters);
}

export function cacheResponse(contentType, parameters, data) {
  if (config.cache=='none') return;
  return cache.response(contentType, parameters, data)
}
/*
