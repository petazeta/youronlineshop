...
export async function respond(request, response) {
  ...
  const responses = new Responses(Node, Linker, User, isAllowedToRead, isAllowedToInsert, isAllowedToModify, makeReport, config.get("db-import-path"))
   ...
   // Specific serversmanager request
  responses.responseAuth.set("get services", async ()=>{
    const {contexts} = await import("../serverinstances.mjs")
    const services = []
    for (const [ckey, cvalue] of contexts) {
      for (const service of cvalue.services) {
        const entry = Object.fromEntries(service)
        entry.context = ckey
        delete(entry.server)
        services.push(entry)
      }
