...
/*
  function loadExtraParents($extraParents){
    if (!is_array($extraParents)) {
      $extraParents=[$extraParents];
    }
    foreach($extraParents as $key=> $val) {
      $extraParents[$key]=new NodeFemale();
      $extraParents[$key]->load($val);
    }
    return $extraParents;
  }
  */
  ...
  switch ($action) {
    ...
    case "add my tree":
      ...
      return $req->db_insertmytree($deepLevel, $extraParents, $myself);
