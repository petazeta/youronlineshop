...
class Theme extends Node{
    ...
    function createTree($data){
      ...
      function innerCreateTree($themes, $parent) {
        ...
        foreach ($themes as $key => $value) {
          ...
          $componentsBranch->props->name="components";
          $myChild->addRelationship($componentsBranch);
          createBranch($themes[$key], $componentsBranch, 'components', true);
  ...
  //creates a list of components files with paths from the theme tree
  //It uses inheritance at components not present at child themes
  function getTpFilesList($myTheme=null, $includeSubs=false){
    ...
    while($myTheme) {
      $themeTps=array();
      $themePaths=array();
      foreach ($myTheme->getRelationship("components")->children as $key => $value) {
