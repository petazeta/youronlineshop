...
const NodeFemaleMixing=Sup => class extends Sup {
  ...
  addChild(obj) { //It doesn't keep the obj previouse parentNode but replace if present
    ...
    if (skey && obj.props[skey] && obj.props[skey]<=this.children.length) {
      this.children=this.children.sort((a,b)=>a.props[skey]-b.props[skey]);
      let i= this.children.length;
      while(i--) {
        if (this.children[i].props[skey] >=obj.props[skey]) {
          this.children[i].props[skey]++;
        }
      }
      this.children.splice(obj.props[skey]-1, 0, obj);
    }
    else this.children.push(obj);
    obj.parentNode=this;
    return obj;
  }
