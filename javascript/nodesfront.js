...
{//FrontEnd facility
  class NodeFront{
    ...
    prerender(tp, params) {
      ...
      scriptElements.forEach(scriptElement => {
        ...
        myScript.textContent="(async ()=>{" + scriptTop + scriptElement.textContent + "})();"; //adding scope (encapsulation) so this variables are local and can't be modified from another scripts.
    ...
    writeProperty(container, property, attribute, onEmptyValueText) {
      ...
      if (Config.onEmptyValueTextOn && !attribute && container.tagName!="INPUT") {
        if (!onEmptyValueText) onEmptyValueText=Node.onEmptyValueText;
    ...
    static async makeRequest(action, params, url) {
      ...
      if (webuser.reqHeaders) fetchParams.headers={...fetchParams.headers, ...webuser.reqHeaders};
    ...
    async request(action, parameters, reduce=false, url) {
      ...
      function reduceParamsNode(params, action) {
      ...
      return Node.makeRequest(action, parameters);
    }
    static async requestMulty(action, dataNodes, parameters, reduce, url) {
      ...
      for (const index in dataNodes) {
        if (parameters[index]) {
          ...
          if (reduce) return [reducedDataNodes, reducedParams];
        }
        else {
          reducedDataNodes[index]=await dataNodes[index].request(action[index], parameters[index], true);
          params[index]={nodeData: reducedDataNodes[index]};
          if (reduce) return reducedDataNodes;
        }
      }
      return Node.makeRequest(action, params, url);
    }
    async loadRequest(action, parameters, url) {
      ...
      switch (action) {
        ...
        case "get my tree":
        if (typeof result=="object") {
          let gender=Node.detectGender(this);
        ...
        if (typeof result=="object") {
          let gender=Node.detectGender(this);
          if (gender=="NodeFemale") this.partnerNode=null;
        ...
        if (typeof result=="number") {
          ...
          //if node female this returns an array of ids and we could set ids of childs
        }
        break;
        case "add my children":
        if (typeof result=="object") {
  ...
  //We add methods
  extendNode(NodeFemaleFront, NodeFemale, ["length", "prototype", "name"]);
  extendNode(NodeFemaleFront.prototype, NodeFemale.prototype, ["constructor"]);
  ...
  //We add methods
  extendNode(NodeMaleFront, NodeMale, ["length", "prototype", "name"]);
  extendNode(NodeMaleFront.prototype, NodeMale.prototype, ["constructor"]);
}