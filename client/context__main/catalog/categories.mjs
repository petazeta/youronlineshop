...
async function setImageView(myNode, viewContainer, size){
  ...
  const imageName = imageNode?.props.imagename || configValues.defaultImg
  imageView.src = configValues.catalogImagesUrlPath + `?size=${size}&image=${imageName}`
...
export function setThumbnail(imageNode){
  ...
  const myImageName=imageNode.props.imagename || configValues.defaultImg
  thumbnailImage.src=pathJoin(configValues.catalogImagesUrlPath, 'small', myImageName)
