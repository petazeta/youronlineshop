...
export async function loginFormView() {
  ...
  selectorFromAttr(selectorFromAttr(logContainer, "data-newuser"), "data-btn").addEventListener("click", async (ev)=>{
    ...
    selectorFromAttr(loginFrame, "data-card-body").appendChild(await rmBoxView(getTemplate, await newFormView(), selectorFromAttr(loginFrame, "data-container")))
  })
  selectorFromAttr(logContainer, "data-form").addEventListener("submit", async function(event) {
    event.preventDefault()
    await loginFormSubm(this)
  })
  return logTp
}
...
async function newFormView() {
  ...
  selectorFromAttr(logContainer, "data-form").addEventListener("submit", async function(event) {
    event.preventDefault()
    await signUpFormSubm(this)
  })
  return logTp
}

async function loginFormSubm(formElement) {
  const logFormTxt = getSiteText().getNextChild("logform")
  ...
  if (checkData instanceof Error) {
    ...
    if (errorKey=="user_name")
      new AlertMessage(getSiteText().getNextChild("logform").getNextChild("userCharError").getlangData(), 3000).showAlert()
    else if (errorKey=="user_password")
      new AlertMessage(getSiteText().getNextChild("logform").getNextChild("pwdCharError").getlangData(), 3000).showAlert()
...
async function signUpFormSubm(formElement) {
  const logFormTxt = getSiteText().getNextChild("logform")
  const checkData = checkValidData({user_name: formElement.elements.user_name.value, user_password: formElement.elements.user_password.value})
  if (checkData instanceof Error) {
    const errorKey = JSON.parse(checkData.message).errorKey
...
async function loginDashboard(){
  ...
  const {cartToCheckOut} = await import("../shop/cart.mjs")
  if (await cartToCheckOut())
    return
  // We need to load user information now
