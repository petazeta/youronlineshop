...
const userMixin=Sup => class extends Sup {
  ...
  static async create(name, password, email){
    const result=await this.constructor.makeRequest("create user", {"user_name": name, "user_password": password, "user_email": email});
  ...
  static async updatePwd(username, password){
    const result = await this.constructor.makeRequest("update user pwd", {"user_name": username, "user_password": password});
