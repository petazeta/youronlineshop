...
export async function setServersButtons(container){
  for (const serv of await Node.makeRequest("get services")) {
    container.appendChild(await serverButtonView(serv))
  }
}

async function serverButtonView(serv){
  const butTp = await getTemplate("serverbut")
  const but = selectorFromAttr(butTp, "data-value")
  but.textContent = servName
