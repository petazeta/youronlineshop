<template>
  <script>
    var editable=thisNode.editable;
    var newNode=thisNode.newNode;
    //Lets add the log event
    var pointer=document.currentScript.parentElement; //We avoid central content elements for log event
    ...
    //to refresh the editable flag
    webuser.addEventListener("log", function(){
      if (thisNode.editable) {
        thisNode.editable=false;
      }
    }, "noeditable", thisNode);
