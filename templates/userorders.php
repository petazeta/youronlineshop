<table class="list">
  <thead>
    ...
    <script>
      if (webuser.getUserType()=="orders administrator" || webuser.isWebAdmin()) {
        (new Node()).appendThis(thisElement, thisElement.querySelector('template')); //We add actions to the header first
      }
    </script>
  </thead>
  <tbody></tbody>
  <script>
    ...
    ordersRel.loadfromhttp({action: myaction, filter: myFilter}).then(function(myNode){
