...
async function displayChildren(parent, pageNum, pageContainer){
  if (!pageContainer)
    pageContainer = parent.pageContainer // container is not childContainer but pageContainer
  else
    parent.pageContainer = pageContainer // set container
  if (!parent.pagination) {
    parent.pagination = new Pagination(parent, async (index)=>{
      await displayChildren(parent, index)
    }, pageSize)
  ...
  const pageView = await parent.pagination.pageView(getTemplate)
  const pageContent = selectorFromAttr(pageView, "data-content") // <ul> element
  for (const myNode of parent.children) {
   pageContent.appendChild(await nodeView(myNode))
  }
  parent.childContainer = pageContent
  ...
  if (parent.children.length == 0  && hasWritePermission()) {
    ...
    // addition button added just inside pagination content
    await setAdditionButton(parent, 1, pageContent, null, onPageAddition)
  }
...
async function linkerView(myNode){
  ...
  selectorFromAttr(container, "data-expander").addEventListener("click", async function(ev) {
    ...
    await displayChildren(myNode, undefined, selectorFromAttr(container, "data-children"))
  })
...
async function setNodeCollectionEdition(myNode, myContainer){
  ...
  await setChangePosButton(myNode, selectorFromAttr(myContainer, "data-admnbuts"), async (increment)=>{
    ...
    if (nextSortOrder > pagination.itemsWindow[1]) {
      await displayChildren(nodeParent, pagination.pageNum + 1)
    }
  ...
  await setDeletionButton(myNode, selectorFromAttr(myContainer, "data-admnbuts"), async (delNode)=>{
    ...
    if (pagination.pageNum < pagination.indexes.length || (pagination.pageNum == pagination.indexes.length && total % pagination.pageSize == 0)) {
      ...
      await displayChildren(nodeParent, pagination.pageNum)
    }
  })
}
...
async function onPageAddition(newNode) {
  ...
  const pageContainer = nodeParent.pageContainer
